...
# 6) 起動時：サイドバーでマスターファイル（CSV/XLSX）を選択またはアップロード可能
...
# 起動時：マスターファイル検出/選択
# =========================================================
def list_master_candidates() -> list[Path]:
    here = Path(".")
    files = [p for p in here.glob("*.*") if p.suffix.lower() in [".csv", ".xlsx"]]
    files = [p for p in files if not p.name.startswith(".") and "_backup" not in p.name and p.parent.name != "_db_backups"]
    pri = {name: i for i, name in enumerate(MASTER_CANDIDATES)}
    files.sort(key=lambda p: pri.get(p.name, 999))
    return files
...
# ---- サイドバー：起動時の読み込み（手動選択対応） ----
st.sidebar.header("起動時の読み込み")

parts_count = count_parts()

def list_master_candidates_sidebar() -> list[str]:
    paths = list_master_candidates()
    return [p.name for p in paths]

cand_labels = list_master_candidates_sidebar()
selected_label = st.sidebar.selectbox(
    "既存ファイルから選択",
    options=cand_labels if cand_labels else ["（見つかりません）"],
    index=0 if cand_labels else 0,
    disabled=not bool(cand_labels),
)
selected_path = (Path(selected_label) if cand_labels else None)

up = st.sidebar.file_uploader("またはアップロード（CSV/XLSX）", type=["csv","xlsx"], help="アップしたファイルは ./uploaded_master.* として保存されます")

load_src: Optional[Path] = None
if up is not None:
    ext = ".csv" if up.name.lower().endswith(".csv") else ".xlsx"
    saved = Path(f"uploaded_master{ext}")
    saved.write_bytes(up.getbuffer())
    load_src = saved
    st.sidebar.success(f"アップロード保存: {saved.name}")
elif selected_path is not None and selected_path.exists():
    load_src = selected_path

if st.sidebar.button("このマスターファイルを読み込む", use_container_width=True, type="primary", disabled=(load_src is None)):
    try:
        dfm = load_master_to_df(load_src)
        ...
        st.sidebar.success(f"{load_src.name} から {n} 件を取り込みました")
    except Exception as e:
        st.sidebar.error(f"読み込み失敗: {e}")

st.sidebar.caption(f"DB内パーツ件数: {parts_count} 件")
...
# =========================================================
# 2) 取り込みタブ：CSV/Excel/テキスト → 候補→確認→確定
# =========================================================
with TAB_INGEST:
